...
// handling action
case class UserRequest[A](claims: Jws[Claims], token: String, request: Request[A]) extends WrappedRequest[A](request)

// Our custom action implementation
class AuthAction @Inject()(bodyParser: BodyParsers.Default, val authService: AuthService)(implicit ec: ExecutionContext)
